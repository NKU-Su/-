...
void MD5Hash(string input[4], __m128i *state)
{
	...
	for (int i = 0; i < max_blocks; i += 1)
	{
		...
		for (int i1 = 0; i1 < 16; ++i1)
		{
			...
			for (int m = 0; m < 4; ++m)
			{
					...
					int offset = i * 64 + i1 * 4;
					x_branch[m] = (uint32_t)msg[offset] 
					            | ((uint32_t)msg[offset + 1] << 8) 
					            | ((uint32_t)msg[offset + 2] << 16) 
					            | ((uint32_t)msg[offset + 3] << 24);
			}
			x[i1] = _mm_set_epi32(x_branch[3], x_branch[2], x_branch[1], x_branch[0]);
		}
	...
	// 下面的处理，在理解上较为复杂
	// 字节顺序反转
	static const __m128i byte_mask = _mm_setr_epi8(
	...
	for (int i = 0; i < 4; ++i)
	{
		state[i] = _mm_shuffle_epi8(state[i], byte_mask);
	}
