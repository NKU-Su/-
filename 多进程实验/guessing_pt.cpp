...
{
    // 决定并行处理的PT数量，可以根据CPU核心数调整
    const int batch_size = 6; // 例如一次处理4个PT
    vector<PT> pts_to_process;
    vector<PT> new_pts_collection;
    vector<vector<string>> guesses_collection;

    // 取出batch_size个PT进行处理
    for (int i = 0; i < batch_size && !priority.empty(); i++) {
        pts_to_process.push_back(priority.front());
    ...
    #pragma omp parallel
    {
        ...
        for (int i = 0; i < pts_to_process.size(); i++) {
            ...
            for (PT new_pt : new_pts) {
                CalProb(new_pt);
                local_new_pts.push_back(new_pt);
            }
        }

        #pragma omp critical
        {
            guesses_collection.push_back(local_guesses);
            new_pts_collection.insert(new_pts_collection.end(), local_new_pts.begin(), local_new_pts.end());
        }
    }

    // 合并所有猜测
    for (auto& g : guesses_collection) {
        guesses.insert(guesses.end(), g.begin(), g.end());
        total_guesses += g.size();
    }

    // 将新PT按概率插入优先队列
    for (PT& pt : new_pts_collection) {
        bool inserted = false;
